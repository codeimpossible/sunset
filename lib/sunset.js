...
module.exports = {
  config: {
    ...
    use_geolocation: {
      ...
      description: `Use your geolocation (via http://ip-api.com) to automatically determine sun
        set/rise times based on the current date. If this setting is checked, the sun set/rise
        settings below are automtically updated.`,
      type: 'boolean',
      default: true,
    },
  ...
  subscriptions: null,
  activate() {
    this.intervalID = setInterval(this.tick.bind(this), TICK_INTERVAL);
    this.setup();
    this.bindEvents();
    this.tick();
  ...
  getLocation() {
    ...
    request.get(url).then((response) => {
      const { lat, lon } = JSON.parse(response);
      const times = SunCalc.getTimes(Date.now(), lat, lon);

      console.log(`sunset: location as returned from ip-api.com (lat, lon): ${lat}, ${lon}`);

      atom.config.set('sunset.when_does_it_get_light', this.getTwentyFourHourTime(times.goldenHourEnd));
      atom.config.set('sunset.when_does_it_get_dark', this.getTwentyFourHourTime(times.dusk));
    }).catch((error) => {
      atom.notifications.addError('Sunset: Error getting geolocation/sun times!', {
  ...
  bindEvents() {
    ...
    this.subscriptions.add(atom.config.observe('sunset.nighttime_ui_theme', (onConfigChange)));
    this.subscriptions.add(atom.config.observe('sunset.nighttime_syntax_theme', (onConfigChange)));
    this.subscriptions.add(atom.config.observe('sunset.daytime_ui_theme', (onConfigChange)));
    this.subscriptions.add(atom.config.observe('sunset.daytime_syntax_theme', (onConfigChange)));
    this.subscriptions.add(atom.config.observe('sunset.use_geolocation', (this.setup.bind(this))));
  },
  setup() {
    this.daytimeThemes = [
  ...
  getTwentyFourHourTime(date) {
    ...
    const roundingFactor = 100.0;

    const minutes = (date.getHours() * hoursToMinutesFactor) + date.getMinutes();

    return Math.round(minutes / roundingFactor) * 100;
  },
  tick() {
    ...
    if (themes.filter(t => currentThemeNames.indexOf(t) > -1).length !== currentThemeNames.length) {
